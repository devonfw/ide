package com.devonfw.tools.ide.dev.functions;
...
public class Functions {
    ...
    public static void doLicenseAgreement() {
        ...
        if (!licenseAgreement.exists()) {
            ...
            System.out.println("This product and its 3rd party components is open-source software and can be used free (also commercially).");
            System.out.println("However, before using it you need to read the license agreement with all involved licenses agreements.");
            System.out.println("With confirming you take notice and agree that there is no warranty for using this product and its 3rd party components.");
            ...
            if (isBatch()) {
                System.err.println("You need to accept these terms of use and all license agreements. Please rerun in interactive (non-batch) mode.");
            }
    ...
    public static void downloadAndExtract(String software, String version, String url, String tmpFile, String targetFolder, String downloadFilename) {
        try {
            URL downloadUrl = new URL(url);
            downloadFilename = (downloadFilename != null && !downloadFilename.isEmpty()) ? downloadFilename : downloadUrl.getPath().substring(downloadUrl.getPath().lastIndexOf('/') + 1);
    ...
    public static void setup(String url, String targetDir, String software, String version, String edition, String code, String os, String arch, String ext, String filename) {
        ...
        try {
            try (Stream<Path> paths = Files.list(urlDirPath)) {
                String finalOs = os;
                downloadUrlFile = paths
                        .filter(Files::isRegularFile)
                        .filter(path -> path.getFileName().toString().contains(finalOs))
                        .map(Path::toString)
                        .findFirst()
                        .orElse(null);
            }
    ...
    private static void createOrUpdateVersionFile(String software, String version, String softwareDir) {
        ...
        if (Files.exists(versionFilePath)) {
            ...
            if (currentVersion.equals(version)) {
                ...
                System.out.println("Updating " + software + " from version " + currentVersion + " to version " + version + "...");
    ...
    public static void updateUrls() {
        ...
        String devonUrls = Environment.get().get("DEVON_URLS");
    ...
    public static void setSoftwareVersion(String software, String version) {
        ...
        if (version != null) {
            if (version.equals("latest")) {
                ...
                String edition = Environment.get().get(softwareEdition);
                if (edition.isEmpty() || edition == null) {
                    edition = software;
                }
                version = getLatestVersion(software, edition);
            ...
            try {
                ...
                lines = lines.stream().filter(line -> !line.startsWith(finalSoftwareVersionVariable)).collect(Collectors.toList());
