...
public class JavaCrawler extends JsonCrawler<JavaJsonObject> {
    ...
    protected void updateVersion(UrlVersion urlVersion) {
        ...
        //https://github.com/adoptium/temurin${major}-binaries/releases/download/${code}/OpenJDK${major}U-jdk_${arch}_${os}_hotspot_${version}.${ext}
        //https://github.com/AdoptOpenJDK/openjdk${major}-binaries/releases/download/${code}/OpenJDK${major}U-jdk_${arch}_${os}_hotspot_${version}.${ext}
        boolean success = doUpdateVersion(urlVersion,baseUrl+major+"-binaries/releases/download/"+code+"/OpenJDK"+major+"U-jdk_x64_windows_hotspot_${version}.zip", OSType.WINDOWS);
        if(!success){
            baseUrl="https://github.com/AdoptOpenJDK/openjdk";
            success = doUpdateVersion(urlVersion,baseUrl+major+"-binaries/releases/download/"+code+"/OpenJDK"+major+"U-jdk_x64_windows_hotspot_${version}.zip", OSType.WINDOWS);
            if(!success) {
                return;
            }
        }
        doUpdateVersion(urlVersion,baseUrl+major+"-binaries/releases/download/"+code+"/OpenJDK"+major+"U-jdk_x64_mac_hotspot_${version}.tar.gz", OSType.MAC);
        doUpdateVersion(urlVersion,baseUrl+major+"-binaries/releases/download/"+code+"/OpenJDK"+major+"U-jdk_x64_linux_hotspot_${version}.tar.gz", OSType.LINUX);
    ...
    protected void collectVersionsFromJson(JavaJsonObject jsonItem, Collection<String> versions) {
        for (JavaJsonVersion item : jsonItem.getVersions()) {
            ...
            versions.add(version); //TODO: Some work has to be done here, replacing stuff
