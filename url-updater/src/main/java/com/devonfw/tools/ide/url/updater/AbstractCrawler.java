...
/**
 * An abstract class representing a web crawler that implements the Updater interface. Contains methods for retrieving
 * response bodies from URLs, updating tool versions, and checking if download URLs work.
 */
public abstract class AbstractCrawler implements Updater {
  ...
  protected UrlRequestResult doCheckIfDownloadUrlWorks(String downloadUrl) {
    ...
    try {
      ...
      URLRequestResult = response.statusCode() >= 200 && response.statusCode() < 400
    ...
    logger.atLevel(logLevel).log("Download url: {} is {} with status code: {}", downloadUrl,
        URLRequestResult.isSuccess() ? "available" : "not available", URLRequestResult.getHttpStatusCode());
