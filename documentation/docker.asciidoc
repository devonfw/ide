...
for our installation, due to it being license free compared to Docker Desktop while offering a similar tool set (please read the document link:docker-desktop-alternative.asciidoc[docker-desktop-alternative.asciidoc] motivating why we added Rancher Desktop to the docker commandlet).
