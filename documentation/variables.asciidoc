...
|*`DOCKER_EDITION`*|e.g. `DOCKERDESKTOP`| If set as `DOCKER` the command `devon docker setup` will setup Docker Desktop globally at the users computer. Otherwise the setup of Rancher Desktop will be started.
