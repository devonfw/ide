...
Note: devonfw-ide now supports autocompletion. Open the terminal (for window use git bash).Type "devon" and hit [Tab].

=== Update
To update your IDE (if instructed by your xref:admin[admin]), you only need to run the following command:
```
devon ide update
```
