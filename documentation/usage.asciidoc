...
=== Update
To update your IDE (if instructed by your xref:architect[Architect]), you only need to run the following command:
```
devon ide update
```
...
== Admin
As an IDE admin for the project you can link:configuration.asciidoc[configure] the `devonfw-ide` ide to meet your needs.
