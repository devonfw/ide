...
Therefore simply follow the link:setup.asciidoc[setup] guide. It is important to notice that this installation gives you the latest published version of the devon-ide with which you can execute commands like eclipse or docker whose scripts are located inside the "${DEVON_IDE_HOME}$/scripts/command, but for commiting code we use a differing folder structure. This will be addressed under xref:Contribution[Contribution].
...
First steps regarding forks or cloning of repos and creating branches, as well as some git commands, can be found inside https://github.com/firstcontributions/first-contributions[ this first contributions guideline].
 To contribute code or text regarding the devon-ide at first you need to fork the https://github.com/devonfw/ide[project IDE] and can then clone your fork into your devon ide installations workspaces-folder (just make sure to execute 'git init' inside 'workspaces' before cloning). Here the https://github.com/firstcontributions/first-contributions[first contributions guideline] can be very helpful.
