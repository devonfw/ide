...
=== doMoveFirstDirectory
Moves a directory resolved from a glob pattern to a target folder.
Example `doMoveFirstDirectory "${target_folder}" "${source_dir}"/*`

|=======================
|*Param*|*Name* |*Description*
|`$1`   |target |The target directory where to move to.
|`$2-$n`|source |The source directory to move. The first existing directory will be moved. If none was found and error is raised.
|=======================
