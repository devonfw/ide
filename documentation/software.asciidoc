...
This subfolder needs to contain the tool in that version for every operating system `«os»` (`windows`, `linux`, or `mac` and `all` for OS independent software such as `maven`).
