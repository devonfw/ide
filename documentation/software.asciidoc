...
This subfolder needs to contain the tool in that version for every operating system `«os»` (`windows`, `linux`, or `mac` amd `all` for OS independent software such as `maven`).
