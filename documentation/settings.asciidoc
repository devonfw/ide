...
Therefore, a technical lead creates a `settings` git repository for the project. He/She creates a "fork" https://github.com/devonfw/ide-settings[devonfw-ide-settings] by adding it as "upstream" origin and pulls from there finally pushing it to the project `settings` git. This also allows to merge changes later  from the official https://github.com/devonfw/ide-settings[devonfw-ide-settings] back into the project specific `settings` "fork".
