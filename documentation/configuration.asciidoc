...
2. `https://github.com/devonfw/ide/blob/master/scripts/src/main/resources/scripts/devon.properties[scripts/devon.properties]` - defaults provided by `devonfw-ide`. Never directly modify this file!
3. `devon.properties` - vendor variables for custom distributions of `link:scripts.asciidoc[devonfw-ide-scripts]`, may e.g. tweak `SETTINGS_PATH` or predefine `SETTINGS_URL`.
...
5. `workspaces/${WORKSPACE}/devon.properties` - optional workspace specific configurations (especically helpful in projects using docker).
