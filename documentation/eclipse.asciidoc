...
|`mirror «url» «name» [«dir»]`|mirror the content of an update-site
|=======================
...
For mirroring you need the `«url»` of the update-site (e.g. http://download.jboss.org/jbosstools/updates/webtools/photon/). Open any CLI in `devonfw-ide` and run the following command. `«name»` stands for the name of the plugin or update-site. It can be chosen arbitrarily.
