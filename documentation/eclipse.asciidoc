...
|`add-plugin «id» [«url»] «plugin name» [optional, creates properties file with given name]`| install an additional plugin
|`remove-plugin «id»[or .properties filename, if exists]`|remove an installed plugin with given id
