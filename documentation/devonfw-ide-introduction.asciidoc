...
It is not another Eclipse or VSCode. It gives you everything you may need to quick start your development, tools like java, nodejs, maven etc., IDEs such as Eclipse, VSCode, IntelliJ etc..

All you need to do is download and extract a zip file and execute its setup, reducing your initial setup time for starting a project from days to minutes.

This is not all you can do with `devon-ide`. It extends the existing shell with helpful commands to automate the provisioning of the development environment.


== Why do you need devonfw-ide?

Usually the team members follows wiki/confluence articles at least once to setup the environment. Many times, these articles might have outdated some days after writing it. Following the step by step instructions, sometimes you may run into errors. Setting up the tools manually are tedious and error prone.

Also there are chances that the team members of a project might have downloaded and using different versions of tools.  Use of different configuration by team members can cause diff-wars. 

The `devonfw-ide` helps you solve these issues, let you download, install and configure the required tools in a single click.

Here are the features you will find through `devonfw-ide`:
...
=== Platforms
`devon-ide` focuses on development of  java, c# and nodejs, but the usage is not limited to these languages.
...
https://github.com/devonfw/ide/tree/master/scripts/src/main/resources/scripts/command[Here] are the list of commands which `devon` supprts.



=== Build-Systems
We support the following build-systems out of the box. Any other build system can be supported by putting some manual effort.
